...
RISCV GNU Toolchain部分已经完成了K扩展的支持并提交了pr到riscv的对应仓库，后续会进行测试用例的添加工作。

B扩展正在添加Binutils部分，预计下周完成。林思南和吴思宇同学正在完成剩下的P扩展指令添加工作，预计本月中旬完成。

陈嘉炜已经完成了zfinx—>z*inx的toolchain修改支持，感谢Kito给予的帮助。
...
binutils:https://github.com/pz9115/riscv-binutils-gdb/tree/riscv-binutils-2.35-k
